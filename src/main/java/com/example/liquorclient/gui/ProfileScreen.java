...
public class ProfileScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawTextWithShadow(textRenderer, Text.literal("Profiles"), x + 12, y + 12, 0xFFFFFFFF);
    ...
    private void drawNormalMode(DrawContext ctx, int x, int y, int mouseX, int mouseY, int accent) {
        ctx.drawTextWithShadow(textRenderer, Text.literal("Current: " + ConfigManager.getCurrentProfile()), x + 12, y + 27, 0xFF9BA8B7);
        ...
        if (renameTarget != null) {
            ...
            ctx.drawTextWithShadow(textRenderer, Text.literal(label), inputX + 6, inputY + 6, newProfileName.isEmpty() ? 0xFF6E7A8A : 0xFFFFFFFF);
            drawButton(ctx, x + PANEL_W - 82, inputY, 70, 20, "Rename", mouseX, mouseY);
            ctx.drawTextWithShadow(textRenderer, Text.literal("Renaming: " + renameTarget), x + 12, y + 74, 0xFFBB8844);
        } else {
        ...
        for (String profile : profiles) {
            ...
            ctx.drawTextWithShadow(textRenderer, Text.literal(profile), x + 22, rowY + 8, current ? 0xFFE9FFE6 : 0xFFC4CAD3);
    ...
    private void drawImportMode(DrawContext ctx, int x, int y, int mouseX, int mouseY, int accent) {
        ctx.drawTextWithShadow(textRenderer, Text.literal("Select a file to import"), x + 12, y + 27, 0xFF9BA8B7);

        int rowY = y + 48;
        List<String> exported = ConfigManager.getExportedProfiles();
        if (exported.isEmpty()) {
            ctx.drawTextWithShadow(textRenderer, Text.literal("No exported profiles found"), x + 22, rowY, 0xFF6E7A8A);
        } else {
            for (String file : exported) {
                ctx.fill(x + 12, rowY, x + PANEL_W - 12, rowY + 24, 0xFF0F151D);
                ctx.fill(x + 12, rowY, x + 14, rowY + 24, 0xFF263241);
                ctx.drawTextWithShadow(textRenderer, Text.literal(file), x + 22, rowY + 8, 0xFFC4CAD3);
