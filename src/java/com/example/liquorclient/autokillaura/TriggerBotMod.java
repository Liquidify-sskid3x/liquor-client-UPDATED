...
public class TriggerBotMod extends Module {
    ...
    private final BooleanSetting enhancedTrigger = new BooleanSetting("Enhanced Trigger (1.8)", true);
    private final NumberSetting hitboxExpand = new NumberSetting("Hitbox Expand", 0.3, 0.0, 2.0);
    ...
    public TriggerBotMod() {
        super("Trigger Bot", Category.COMBAT, "(marlow faking t bot for 2 hours she used this trust)");
        addSetting(minDelay);
        addSetting(maxDelay);
        addSetting(combat18);
        addSetting(autoblock18);
        addSetting(enhancedTrigger);
        addSetting(hitboxExpand);
        addSetting(minCps);
        addSetting(maxCps);
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            ...
            if (mc.crosshairTarget instanceof EntityHitResult entityHit) {
                ...
                if (target.isAlive() && target instanceof PlayerEntity) {
                    if (FriendManager.isFriend((PlayerEntity) target)) {
                        debugSkip("friend", delay, sinceLast, target.getName().getString());
                        return;
                    }
                ...
                // If legacy 1.8 mode and enhanced trigger enabled, attempt an expanded-target lookup
                if (legacyMode && enhancedTrigger.getValue()) {
                    double expand = Math.max(0.0, hitboxExpand.getValue());
                    double baseRange = 3.0; // vanilla melee reach baseline
                    double searchRange = baseRange + expand;
                    PlayerEntity found = mc.world.getEntitiesByClass(PlayerEntity.class, mc.player.getBoundingBox().expand(searchRange), e ->
                            e != mc.player && e.isAlive() && mc.player.squaredDistanceTo(e) <= searchRange * searchRange && !FriendManager.isFriend(e)
                    ).stream()
                            .filter(e -> {
                                Vec3d look = mc.player.getRotationVec(1.0f);
                                Vec3d playerPos = new Vec3d(mc.player.getX(), mc.player.getY(), mc.player.getZ());
                                Vec3d toEnt = new Vec3d(e.getX(), e.getY(), e.getZ()).subtract(playerPos).normalize();
                                double dot = look.dotProduct(toEnt);
                                return dot > 0.75; // roughly within ~41 degrees
                            })
                            .findFirst().orElse(null);

                    if (found != null) {
                        if (focusTarget.getValue() && lastTarget != null && lastTarget.isAlive() && found != lastTarget) {
                            if (mc.player.squaredDistanceTo(lastTarget) < 40.0) {
                                debugSkip("focusing old target", delay, sinceLast, found.getName().getString());
                                return;
                            } else {
                                lastTarget = null;
                            }
                        ...
                        if (found.isAlive()) {
                            ...
                            if (legacyMode || cooldown >= 1.0f) {
                                if (!performVanillaAttack(legacyMode)) {
                                    debugSkip("doAttack rejected (enhanced)", delay, sinceLast, found.getName().getString());
                                    return;
                                }
                                debugAttempts++;
                                lastTarget = found;
                                lastAttackTime = now;
                                debugAttack(found.getName().getString(), delay, sinceLast, cooldown, legacyMode);
                                return;
                            } else {
                                debugSkip("cooldown (enhanced)" + format(cooldown), delay, sinceLast, found.getName().getString());
                                return;
                            }
                        }
                    }
    ...
    private void updateSettingVisibility() {
        ...
        enhancedTrigger.setVisible(use18);
        hitboxExpand.setVisible(use18 && enhancedTrigger.getValue());
    }
