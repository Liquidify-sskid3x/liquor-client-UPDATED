...
public class ClickGui extends Screen {
    ...
    private void drawCategories(DrawContext ctx, int x, int y, double mx, double my, boolean leftDown, int accent) {
        ...
        for (Category cat : Category.values()) {
            ...
            CustomFontRenderer.drawString(ctx, textRenderer, name, rowX + 8, cy + 7,
    ...
    private void drawUtilityButtons(DrawContext ctx, int x, int y, double mx, double my, boolean leftDown) {
        int rowX = x + 8;
        int rowW = SIDEBAR_W - 16;
        int profileY = y + PANEL_H - 84;
        int hudY = y + PANEL_H - 58;
        int friendsY = y + PANEL_H - 32;

        drawSidebarButton(ctx, rowX, profileY, rowW, 20, "Profiles", mx, my);
        drawSidebarButton(ctx, rowX, hudY, rowW, 20, "HUD Editor", mx, my);
        drawSidebarButton(ctx, rowX, friendsY, rowW, 20, "Friends", mx, my);
        ...
        if (leftDown && !lastLeftDown) {
            ...
            } else if (isInside(mx, my, rowX, friendsY, rowW, 20)) {
    ...
    private void drawSidebarButton(DrawContext ctx, int x, int y, int w, int h, String label, double mx, double my) {
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, label, x + w / 2 - labelW / 2, y + 6, 0xFFC4CAD3);
    }
    ...
    private void drawModules(DrawContext ctx, int x, int y, double mx, double my, boolean leftDown, boolean rightDown,
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, titleCase(selectedCategory.name()), startX, y + 12,
                0xFFFFFFFF);
        CustomFontRenderer.drawString(ctx, textRenderer, "left toggle / right settings", startX, y + 26, 0xFF687789);
        ...
        for (Module module : modules) {
            ...
            CustomFontRenderer.drawString(ctx, textRenderer, module.getName(), startX + 9, cy + 7,
                    enabled ? 0xFFE9FFE6 : 0xFFC4CAD3);

            String state = enabled ? "ON" : "OFF";
            int stateW = CustomFontRenderer.getStringWidth(textRenderer, state);
            int pillX = startX + contentW - (maxScroll > 0 ? 6 : 0) - 42;
            ctx.fill(pillX, cy + 5, pillX + 32, cy + 18, enabled ? 0xFF1F6B32 : 0xFF2A3441);
            CustomFontRenderer.drawString(ctx, textRenderer, state, pillX + 16 - stateW / 2, cy + 8, 0xFFFFFFFF);
    ...
    private void drawSearchBar(DrawContext ctx, int x, int y, int w, double mx, double my, boolean leftDown,
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, label, x + 7, y + 5,
    ...
    private void drawSettings(DrawContext ctx, int x, int y, double mx, double my, boolean leftDown, int accent) {
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, "< Back", startX + 8, cy + 4, 0xFFB9C4D3);
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, settingsModule.getName(), startX, y + 38, 0xFFFFFFFF);

        String desc = settingsModule.getDescription();
        if (!desc.isEmpty()) {
            CustomFontRenderer.drawString(ctx, textRenderer, desc, startX, y + 50, 0xFF8899AA);
    ...
    private int drawNumberSetting(DrawContext ctx, NumberSetting setting, int x, int y, int w, double mx, double my,
        ...
        int valueW = CustomFontRenderer.getStringWidth(textRenderer, value);

        CustomFontRenderer.drawString(ctx, textRenderer, setting.getName(), x, y, 0xFFE5EBF2);
        CustomFontRenderer.drawString(ctx, textRenderer, value, x + w - valueW, y, 0xFF91A0B4);
    ...
    private int drawColorSetting(DrawContext ctx, ColorSetting setting, int x, int y, int w, double mx, double my,
        ...
        int hexW = CustomFontRenderer.getStringWidth(textRenderer, hex);

        CustomFontRenderer.drawString(ctx, textRenderer, setting.getName(), x, y, 0xFFE5EBF2);
        ctx.fill(x + w - 38, y - 2, x + w - 18, y + 10, color);
        drawOutline(ctx, x + w - 38, y - 2, 20, 12, 0xFF2A3748);
        CustomFontRenderer.drawString(ctx, textRenderer, hex, x + w - hexW, y + 14, 0xFF91A0B4);
    ...
    private int drawBooleanSetting(DrawContext ctx, BooleanSetting setting, int x, int y, int w, double mx, double my,
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, setting.getName(), x, y + 4, 0xFFE5EBF2);
    ...
    private int drawKeybindSetting(DrawContext ctx, KeybindSetting setting, int x, int y, int w, double mx, double my,
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, "Keybind", x, y + 5, 0xFFE5EBF2);
        ...
        int labelW = CustomFontRenderer.getStringWidth(textRenderer, label);
        CustomFontRenderer.drawString(ctx, textRenderer, label, buttonX + buttonW / 2 - labelW / 2, y + 5,
                0xFFFFFFFF);
    ...
    private void drawTooltip(DrawContext ctx, String text, int mx, int my) {
        ...
        int lineH = CustomFontRenderer.getFontHeight() + 2;
        int totalH = lines.size() * lineH + 4;
        int maxLineW = 0;
        for (String line : lines) {
            int lw = CustomFontRenderer.getStringWidth(textRenderer, line);
        ...
        int ly = ty + 1;
        for (String line : lines) {
            CustomFontRenderer.drawString(ctx, textRenderer, line, tx + 4, ly, 0xFFCCD4DE);
            ly += lineH;
        }
    }
    ...
    private List<String> wrapText(String text, int maxPx) {
        ...
        if (CustomFontRenderer.getStringWidth(textRenderer, text) <= maxPx) {
            lines.add(text);
            return lines;
        }
        ...
        for (String word : words) {
            ...
            if (CustomFontRenderer.getStringWidth(textRenderer, test) > maxPx && !current.isEmpty()) {
