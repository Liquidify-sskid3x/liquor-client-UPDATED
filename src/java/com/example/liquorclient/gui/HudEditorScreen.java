...
public class HudEditorScreen extends Screen {
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        ctx.drawCenteredTextWithShadow(textRenderer, CustomFontRenderer.withFont("HUD Editor"), width / 2, 14, 0xFFFFFFFF);
        ctx.drawCenteredTextWithShadow(textRenderer, CustomFontRenderer.withFont("Drag elements. Clamping prevents off-screen."), width / 2, 28, 0xFF9BA8B7);
    ...
    public boolean mouseClicked(Click click, boolean doubled) {
        ...
        if (click.button() == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
            if (cycleHudAnchor(mouseX, mouseY)) return true;
    ...
    private boolean cycleHudAnchor(double mouseX, double mouseY) {
        record HudEntry(String name, int x, int y, int w, int h, java.util.function.IntConsumer setAnchor, java.util.function.IntSupplier getAnchor) {}

        HudEntry[] entries = {
            new HudEntry("Info", ArrayListHud.getInfoX(), ArrayListHud.getInfoY(), ArrayListHud.getInfoWidth(), ArrayListHud.getInfoHeight(), ArrayListHud::setInfoAnchor, ArrayListHud::getInfoAnchor),
            new HudEntry("ArrayList", getArrayX(), ArrayListHud.getArrayY(), ArrayListHud.getMaxArrayWidth(), ArrayListHud.getArrayHeight(), ArrayListHud::setArrayAnchor, ArrayListHud::getArrayAnchor),
            new HudEntry("Armor", ArrayListHud.getArmorX(), ArrayListHud.getArmorY(), ArrayListHud.getArmorWidth(), ArrayListHud.getArmorHeight(), ArrayListHud::setArmorAnchor, ArrayListHud::getArmorAnchor),
            new HudEntry("Potions", ArrayListHud.getPotionX(), ArrayListHud.getPotionY(), ArrayListHud.getPotionWidth(), ArrayListHud.getPotionHeight(), ArrayListHud::setPotionAnchor, ArrayListHud::getPotionAnchor),
            new HudEntry("Keystrokes", ArrayListHud.getKeystrokesX(), ArrayListHud.getKeystrokesY(), ArrayListHud.getKeystrokesWidth(), ArrayListHud.getKeystrokesHeight(), ArrayListHud::setKeystrokesAnchor, ArrayListHud::getKeystrokesAnchor),
            new HudEntry("CPS", ArrayListHud.getCpsX(), ArrayListHud.getCpsY(), ArrayListHud.getCpsWidth(), ArrayListHud.getCpsHeight(), ArrayListHud::setCpsAnchor, ArrayListHud::getCpsAnchor),
        };

        for (HudEntry e : entries) {
            if (isInside(mouseX, mouseY, e.x(), e.y(), e.w(), e.h())) {
                int next = ArrayListHud.cycleAnchor(e.getAnchor().getAsInt());
                e.setAnchor().accept(next);
                NotificationManager.push(e.name(), switch (next) {
                    case ArrayListHud.ANCHOR_LEFT -> "Anchored to left";
                    case ArrayListHud.ANCHOR_DOWN -> "Anchored to bottom";
                    case ArrayListHud.ANCHOR_RIGHT -> "Anchored to right";
                    default -> "Anchored to top";
                });
                return true;
            }
        }
    ...
    private void drawPreview(DrawContext ctx, String name, int x, int y, int w, int h, int accent) {
        ...
        CustomFontRenderer.drawString(ctx, textRenderer, name, x + 4, y + 4, 0xFFFFFFFF);
    }
