...
public class KeystrokesMod extends Module {
    ...
    private void drawKey(DrawContext ctx, MinecraftClient mc, String label, KeyBinding key, int x, int y, int w, int h) {
        ...
        int labelW = CustomFontRenderer.getStringWidth(mc.textRenderer, label);
        CustomFontRenderer.drawString(ctx, mc.textRenderer, label, x + w / 2 - labelW / 2, y + h / 2 - 4, color);
    }
