...
public class ESPManager {
    ...
    private static void renderLivingEntities(DrawContext ctx, MinecraftClient mc, RenderState state, EspMod esp) {
        ...
        for (Entity entity : mc.world.getEntities()) {
            ...
            int color = espColor(esp, isPlayer, living, System.currentTimeMillis());
    ...
    private static int espColor(EspMod esp, boolean isPlayer, LivingEntity entity, long now) {
        if (esp.shouldUseRainbow()) {
            return rainbowColor(now);
        }
        if (isPlayer && entity instanceof net.minecraft.entity.player.PlayerEntity player && FriendManager.isFriend(player)) {
            return 0xFF44FF44;
        }
    ...
    private static void drawCenteredName(DrawContext ctx, MinecraftClient mc, String name, ScreenBox box, int color) {
        int nameW = CustomFontRenderer.getStringWidth(mc.textRenderer, name);
        ctx.drawTextWithShadow(mc.textRenderer, CustomFontRenderer.withFont(name), box.x + box.w / 2 - nameW / 2, box.y - 10, color);
    }
    ...
    private record RenderState(
        ...
        static RenderState create(MinecraftClient mc, float tickDelta) {
            ...
            float fov = ((com.example.liquorclient.mixin.GameRendererAccessor) mc.gameRenderer).liquor$getFov(camera, tickDelta, true);
            ...
            Vec3d cameraPos = ((com.example.liquorclient.mixin.CameraAccessor) camera).liquor$getPos();
